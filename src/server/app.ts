  ...
  for (const modelName of modelsToTry) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (isUnavailable) {
          // Transient high demand spike: wait briefly before retry or next model
          if (attempt === 0) {
...
app.post(['/api/generate-project', '/generate-project'], async (req, res) => {
  res.setHeader('Content-Type', 'application/json');
...
app.get(['/api/paypal/config', '/paypal/config'], (_req, res) => {
  res.setHeader('Content-Type', 'application/json');
...
app.post(['/api/paypal/create-order', '/paypal/create-order'], async (req, res) => {
  res.setHeader('Content-Type', 'application/json');
  try {
    const authUser = await authenticateUserFromRequest(req);
...
app.post(['/api/paypal/capture-order', '/paypal/capture-order'], async (req, res) => {
  res.setHeader('Content-Type', 'application/json');
  try {
